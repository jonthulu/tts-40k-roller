...
local function resetLastAttack()
    lastAttack = {
      state = nil,
      hitSkill = nil,
      hitValues = nil,
...
local function throwD6(dieCount, skill)
...
local function reRollD6(dieCount, skill, previousValues)
    local thrownDice = {0, 0, 0, 0, 0, 0}
    local results = {hits = 0, misses = 0}
...
local function playerToPrintColor(player)
    return player
end
...
local function printDieValues(prefix, dieValues, player)
    local output = ''
